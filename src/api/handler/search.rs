...
impl SearchAPI {
    ...
    pub async fn token_freq(mut c: Context<SiteContext>) -> Context<SiteContext> {
        c.c.health.add_search_access_count();
        let req = c.req.body_de_struct::<schema::search::TokenFreqReq>().await;
        let result = match req {
            Ok(r) => c.c.ssr.search_page.search_api_token_freq(r).await,
            Err(_e) => {
                c.res.text("Bad Request: Failed to parse request body");
                c.res.set_status(400);
                return c;
    ...
    pub async fn token_freq_get(mut c: Context<SiteContext>) -> Context<SiteContext> {
        c.c.health.add_search_access_count();
        let base = c.req.path.get_field("*").unwrap_or("".into());
        let query = c.req.path.path.splitn(2, '?').nth(1).unwrap_or("");
        let url = if query.is_empty() {
        ...
        let query = schema::search::TokenFreqReq {
            url: url.to_string(),
        };
        let result = c.c.ssr.search_page.search_api_token_freq(query).await;
